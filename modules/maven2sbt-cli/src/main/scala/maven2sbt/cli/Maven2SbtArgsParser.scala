...
object Maven2SbtArgsParser {
  ...
  private val scalaBinaryVersionArg: Parse[Option[ScalaBinaryVersion.Name]] = flag[String](
    ...
    metavar("<scala-binary-version-name>") |+| description(
  ...
  private val paramsNameArg: Parse[Props.PropsName] = flag[String](
    long("props-name"),
    metavar("<props-name>") |+| description(
      "properties object name (e.g. 'props' in `lazy val props = new {}`) (default: props)",
    ),
  ).default("props").map(Props.PropsName.apply)

  private val libsNameArg: Parse[Libs.LibsName] = flag[String](
    long("libs-name"),
    metavar("<libs-name>") |+| description(
      "The name of the object containing all the libraries to re-use (e.g. 'libs' in `lazy val libs = new {}`) (default: libs)",
    ),
