...
object Gens {
  ...
  def genProjectInfo: Gen[ProjectInfo] =
    for {
      groupId    <- genGroupId
  ...
  def genRepository: Gen[Repository] =
    for {
      id   <- genRepositoryId
  ...
  def genRepositoryWithEmptyName: Gen[Repository] =
    for {
      id <- genRepositoryId
  ...
  def genRepositoryWithNoName: Gen[Repository] =
    for {
      id  <- genRepositoryId
      url <- genRepositoryUrl
    } yield Repository(id.some, none[RepoName], url)

  def genRepositoryWithEmptyIdEmptyName: Gen[Repository] =
    for {
      url <- genRepositoryUrl
  ...
  def genMavenPropertyNameWithPropNamePair: Gen[(MavenProperty.Name, Prop.PropName)] = for {
    nameList      <-
      Gen.string(Gens.genCharByRange(TestUtils.ExpectedLetters), Range.linear(1, 10)).list(Range.linear(1, 10))
    delimiterList <-
      Gen
        .string(
  ...
  def genMavenProperty: Gen[MavenProperty] =
    for {
      nameList <-
        Gen.string(Gens.genCharByRange(TestUtils.ExpectedLetters), Range.linear(1, 10)).list(Range.linear(1, 10))
  ...
  def genMavenPropertyAndPropPair: Gen[(MavenProperty, Prop)] =
    for {
      nameList <-
        Gen.string(Gens.genCharByRange(TestUtils.ExpectedLetters), Range.linear(1, 10)).list(Range.linear(1, 10))
  ...
  def genDependencyWithNonEmptyExclusions: Gen[Dependency] =
    for {
      groupId    <- genGroupId
      artifactId <- genArtifactId
      version    <- genVersion
